...
This shift led to the **targeted introduction of Knowledge-Based models**, which utilize engineering equations and pre-established design limits to identify known failure patterns. Unlike generic data-driven alerts, these models direct the user's attention to specific, actionable issues (e.g., distinguishing "Lube Oil Filter Clogging" from general "Vibration Anomaly"). This approach helped to standardize the definition of a "success case" from simply "catching an anomaly" to "preventing a specific failure mode."
